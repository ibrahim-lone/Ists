...
Permutation FindPosition(const Permutation &v, int t, const unordered_map<int, int> &v_inv) {
    int n = v.size();
    Permutation id(n);
...
Permutation Parent1(const Permutation &v, int t) {
    int n = v.size();
    unordered_map<int, int> v_inv = compute_inverse(v);
    int vn = v[n - 1];
    int vn_1 = v[n - 2];
    ...
    if (vn == n) {
        return (t != n - 1) ? FindPosition(v, t, v_inv): Swap(v, vn_1, v_inv);
    } else {
...
int main() {
    ...
    // Reserve space for all permutations
    Permutation base(n);
...
for (const auto &v : permutations) {
    if (v == base) continue; // root has no parent
    for (int t = 1; t <= n - 1; ++t) {
        Permutation parent = Parent1(v, t);
